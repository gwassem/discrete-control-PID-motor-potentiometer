...
Kp = 0.0079673;      % GANHO PROPORCIONAL
Ki = 0.00034421;     % GANHO INTEGRAL (Tente reduzir este se houver muito overshoot ou windup)
Kd = 0;             % GANHO DERIVATIVO

pwmMax = 0.8;    % Duty Cycle Máximo (100% PWM)
pwmMinAtivacao = 0.1; % PWM mínimo para superar atrito.
...
try
    while toc(tempoInicioControle) < duracaoMaxControle
        ...
        if abs(erro) < erroZonaMorta 
            ...
            rawPidOutput = termoP + Ki * integralErro + termoD; % Usa o integralErro atual
            ...
            % Opcional: Ainda pode adicionar um limite "seguro" para integralErro, como uma proteção extra
            % integralErro = max(min(integralErro, X), -X); % Onde X é um valor razoável para o seu sistema

            % Saída final do controlador PID, considerando o termo integral atualizado
            pidOutput = termoP + Ki * integralErro + termoD;
